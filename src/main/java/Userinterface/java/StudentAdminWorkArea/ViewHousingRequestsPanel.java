 ...
 */
public class ViewHousingRequestsPanel extends javax.swing.JPanel {

    /**
     * Creates new form ManageUniversities
     */
    public ViewHousingRequestsPanel() {
        initComponents();
        ObjectSet<HousingRequest> housing = Database.db.get(HousingRequest.class);
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
        for(HousingRequest c :housing){
            String pend = "Pending";
            if(c.isResolved())
            {
                pend = "Resolved";
            }
            model.addRow(new Object[]{c.getName(),c.getEmail(),c.getNumber(),pend});
        }
    }
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Name", "Email ID", "Contact Number", "Status"
            }
        ) {
        ...
        jPanel1.add(acceptButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 460, 140, 60));
        ...
        jPanel1.add(declineButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 460, -1, 60));

        add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 1420, 1120));
    ...
    private void acceptButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_acceptButton1MouseClicked
        ...
        HousingRequest hr = new HousingRequest(em,null,null);
        ObjectSet<User> user = Database.db.get(u);
        ObjectSet<HousingRequest> request = Database.db.get(hr);
        if(user.hasNext() && request.hasNext())
        {
            ...
            if(!hr.isResolved())
            {
                u.setHousingPermission(true);
                hr.setResolved(true);
                Database.db.store(u);
                Database.db.store(hr);
                model.setValueAt("Resolved",row,3);
            }
        }
    ...
    private void declineButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_declineButton1MouseClicked
        ...
        if(user.hasNext() && request.hasNext())
        {
            ...
            if(!hr.isResolved())
            {
                u.setHousingPermission(false);
                Database.db.store(u);
                Database.db.delete(hr);
                model.setValueAt("Declined",row,3);
            }
        }
